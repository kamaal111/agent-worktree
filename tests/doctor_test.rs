...
"#,
    ...
    assert!(messages
        .iter()
        .any(|message| message.contains("custom workspaceMount")));
    assert!(messages
        .iter()
        .any(|message| message.contains("container_name")));
    assert!(messages
        .iter()
        .any(|message| message.contains("host network")));
    assert!(messages
        .iter()
        .any(|message| message.contains("fixed host port")));
    assert!(messages
        .iter()
        .any(|message| message.contains("Docker socket")));
    assert!(messages
        .iter()
        .any(|message| message.contains("fixed global name")));
    assert!(messages
        .iter()
        .any(|message| message.contains("external and may be shared")));
